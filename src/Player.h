...
Player::Player():gravity(3),speedMin(2.5),jumpHeightMin(-6.5){
    initTexture();
    initSprite();
    initPhysics();
    pro=new Bullet;
...
void Player::movement(char tempColHor, char tempColVer){
 //   sf::Vector2f a=sprite.getPosition();
  ...
  //  if(sf::Keyboard::isKeyPressed)
    if(sf::Keyboard::isKeyPressed(sf::Keyboard::LShift)&& isSprinting){
        isSprinting=false;
    }
    else if(sf::Keyboard::isKeyPressed(sf::Keyboard::LShift) && isSprinting==false){
        isSprinting=true;
    }
...
sf::FloatRect Player::getPlayerGlobal(){
    return sprite.getGlobalBounds();
}
sf::FloatRect Player::getObjectGlobal(){
